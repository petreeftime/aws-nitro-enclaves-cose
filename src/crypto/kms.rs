...
use aws_sdk_kms::{
    ...
    types::Blob, Client, types::SdkError
};

use crate::{
    crypto::openssl_pkey::ec_curve_to_parameters,
    crypto::{SigningPrivateKey, SigningPublicKey, SignatureAlgorithm, MessageDigest},
    error::CoseError,
};
    ...
    pub fn new_with_public_key(
        ...
        let public_key = match public_key {
            ...
            None => {
                ...
                PKey::public_key_from_der(public_key.as_ref()).map_err(|e| CoseError::SignatureError(Box::new(e)))?
            }
        };
...
impl SigningPublicKey for KmsKey {
    ...
    fn verify(&self, data: &[u8], signature: &[u8]) -> Result<bool, CoseError> {
        if self.public_key.is_some() {
            ...
            let sig = EcdsaSig::from_private_components(r, s).map_err(|e| CoseError::SignatureError(Box::new(e)))?;
            let sig = sig.to_der().map_err(|e| CoseError::SignatureError(Box::new(e)))?;
...
impl SigningPrivateKey for KmsKey {
    fn sign(&self, data: &[u8]) -> Result<Vec<u8>, CoseError> {
        ...
        let signature =
            EcdsaSig::from_der(signature.as_ref()).map_err(|e| CoseError::SignatureError(Box::new(e)))?;
